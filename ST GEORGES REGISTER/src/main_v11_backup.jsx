...
function Dashboard({profile,admin,settings}){
 ...
 // IMPORTANT: Do not use Supabase range pagination for the dashboard.
 // The project can return HTTP 400 when a second range starts at 1000.
 // Instead, split the school into grades (each grade is safely below the API row cap)
 // and split attendance IDs into small batches.
 async function loadDashboardLearners(filters={}){
   const grades=filters.id?[null]:(filters.grade?[filters.grade]:(teacher?[profile.grade||'']:ALL_GRADES));
   const chunks=[];
   for(const g of grades){
     ...
     // Each school grade is well below the normal API row limit. Keep the
     // query scoped to one grade so the Dashboard never requests a 1,000+
     // row page or a huge learner-id IN clause.
     const {data,error}=await q.order('full_name');
     if(error) return {data:null,error};
     chunks.push(...(data||[]));
     if(filters.id && chunks.length) break;
   }
 ...
 useEffect(()=>{
   (async()=>{
     setError('');
     let countQuery=supabase.from('learners').select('id',{count:'exact',head:true}).eq('active',true);
     if(teacher){
       countQuery=countQuery.eq('grade',profile.grade||'');
       if(profile.stream) countQuery=countQuery.eq('stream',profile.stream);
       else countQuery=countQuery.is('stream',null);
     }
     const {count,error:countError}=await countQuery;
     if(countError){setError(countError.message);return;}
     setDbTotal(count||0);
     const {data,error}=await loadDashboardLearners({});
     if(error){setError(error.message);return;}
     setAllLearners(data||[]);
     if((data||[]).length!==(count||0)){
       setError(`Dashboard loaded ${(data||[]).length} learners, but Supabase reports ${count||0} active learners.`);
     }
   })();
 },[profile,teacher]); const selectedSection=useMemo(()=>{
 ...
 async function load(){
   const filters={};
   if(view==='grade' && selectedGrade) filters.grade=selectedGrade;
   if(view==='stream'){
     if(selectedGrade) filters.grade=selectedGrade;
     if(selectedStream) filters.stream=selectedStream;
   }
   if(view==='individual' && selectedLearner) filters.id=selectedLearner;

   const {data,error}=await loadDashboardLearners(filters);
   if(error){setError(error.message);return;}
   setLearners(data||[]);
   if(view==='whole' && !teacher && dbTotal && (data||[]).length!==dbTotal){
     setError(`Dashboard loaded ${(data||[]).length} learners, but Supabase reports ${dbTotal} active learners.`);
   }

   const ids=(data||[]).map(x=>x.id).filter(Boolean);
   if(!ids.length){setAttendance([]);return;}

   // Fetch attendance in small ID batches to avoid an oversized HTTP request.
